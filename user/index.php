...
<div class="py-5 bg-light">
    <div class="container">
        ...
        <div class="col-md-6">
                <h3 class="text-dark">Recommended Posts</h3>
        </div>
               <?php       
                if (isset($_SESSION['users'])){
                $loggedinUser=$_SESSION['users'];
                // it works as simple collaborative filtering
                $query="select distinct title,slug,description,image,date from rating NATURAL JOIN news where uid IN (select uid from rating where nid in (select nid from rating where uid=$loggedinUser) and uid!=$loggedinUser)";
                $result=mysqli_query($conn,$query);
                
                if($result->num_rows===0){?>
                <div class="col-md-12">
                    <h6 class="text-dark"> No Recommended Posts Found</h3>
                </div>
                <?php
            }
                ...
                <div class="row">
                    <div class="mb-4 col-md-12">
                            <a class="text-decoration-none" href=" post.php?title=<?= $row['slug'];?>">
